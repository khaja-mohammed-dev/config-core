...
/**
 * Registers {@code POST /internal/config/update}, but only when {@code config-core.internal.secret}
 * is set: the endpoint is off unless explicitly configured, never open by default.
 */
...
@ConditionalOnBean(ConfigWriter.class)
...
public class ConfigCoreEndpointAutoConfiguration {
    ...
    @Bean
    InternalConfigController configCoreInternalConfigController(ConfigWriter writer, ConfigCoreProperties properties) {
        ...
        return new InternalConfigController(writer, secret);
    }
}
